...
- **当前进度**：已建立 npm workspace、统一 build/test/typecheck/ci 入口和基于 TypeScript AST 的包边界 lint；门禁要求 engine 零运行时依赖且只引用自身模块、web 不依赖 Node/server、server 不反向依赖 web。`scripts/dev.mjs` 已按平台选择 `npm`/`npm.cmd`，`.github/workflows/ci.yml` 配置 Node 20 的 macOS/Windows `npm ci + lint + typecheck + test + build` 矩阵。仓库新增包含 workflow 与 content-only 两个独立项目的 `examples/multi-skill-workspace`，稳定 skillId 不重复，图与文档引用通过确定性测试。可见 macOS Google Chrome 已从空数据目录在页面创建 Workspace、两次原地打开、切换两个 Skill、查看两张 2D 图和 390px 文档页；Canvas 均有非空像素且取景不贴边，源文件 hash 前后一致，控制台错误和失败响应均为 0。证据在 `.skill-designer-dev/chrome-artifacts/example-workspace/verification.json`。CI 配置尚未在真实 Windows runner 执行，因此本项保持进行中。
...
- **当前进度**：普通运行与 Benchmark 已统一从 active revision 的不可变 Snapshot 生成单 Skill RuntimeArtifact，并冻结图、内容 hash 与初始变量。运行指纹使用 `sha256(projectContentHash + canonical initialVariables)`：对象键排序后序列化，因此同一 JSON 输入不受键顺序影响，输入变化或项目内容变化都会产生不同总指纹；旧 Artifact 读取保留兼容回填。Workspace 内切换 Skill 和 active revision 漂移均不改变既有运行，页面可查看项目内容 hash、输入 hash、冻结输入、当前输出和最近模型回执。尚缺三系统同字节指纹矩阵与 Artifact 过期清理策略，因此保持进行中。
...
- **完成情况**：已实现 Markdown 文件树、路径检索、编辑/安全预览、新建、重命名和删除、节点 `doc + docAnchor` 绑定、引用计数与精确切片预览。`SKILL.md` 受保护；`docs.rename` 自动同步节点路径并保留锚点，`docs.delete` 在明确列出受影响节点后清除绑定，文档和 graph 共同进入一个 ChangeSet 的预览、确认、snapshot、revision 与多文件回滚。确认期间源字节、目标占用或图引用变化会标记冲突。存储测试和可见 Chrome 已验证确认前零写入、重命名内容保留、引用同步、删除警告/解绑、路径搜索及桌面/移动布局。100 篇关联文档的真实原地项目在 macOS arm64 Chrome 150 中实际渲染 101 项（含 `SKILL.md`）耗时 1029ms，首/中/末搜索为 177/16/32ms，打开末篇 229ms，均低于专项脚本的 5000/1000/2000ms 上限；环境与截图证据保存于 `.skill-designer-dev/chrome-artifacts/document-scale-100.json`。跨平台总体性能仍由 T39 验收。
...
- **当前进度**：已实现 `static-v1` 原生图/明确 Markdown 流程解析、保守 `content-only` 降级、候选节点与关系、置信度、来源行与片段、未决问题、接受/修改/拒绝、服务端 lint、reviewRevision/digest 乐观锁和冻结源文件重解析。候选审阅复用原项目风格的 3D/2D 力导向图谱；人工修改后重解析形成持久化 `manual-vs-reparse` 冲突，只能显式保留人工版本或采用新解析结果。冲突、未保存修改和 lint error 均阻断确认，确认前项目不存在，确认后写入已裁决图且不改写源 Markdown。engine/store/HTTP 定向测试和可见 Chrome 已实际完成 3D/2D 非空画布、证据展示、节点修改、重解析冲突、移动端裁决、保留人工版本和最终图核对；证据在 `import-review.json`、`import-review-3d-desktop.png`、`import-review-conflict-desktop.png`、`import-review-conflict-mobile.png`。
...
- **当前进度**：已实现结构化 DiagnosisRepairOption、DiagnosisRepairRecord 和 `unverified/verified/failed` 状态。仅当报告与当前 Skill 内容指纹仍精确一致、拒绝目标节点已存在且当前没有直接流程边时，才生成单条 `graph.edge.create` ChangeSet；缺失节点和证据不足候选不会生成猜测性修改。页面复用图 diff，用户确认前项目字节不变，确认后记录 changeSetId/appliedRevision 并保持 `unverified`。runtime 验证只接受修复后 revision 的新 run，Trace 必须实际经过新增边并到达 completed 才标记 `verified`，再次出现同一拒绝则标记 `failed`，未覆盖修复边时返回 inconclusive；回放和旧 run 均不能验证。Benchmark 来源报告另有专用启动与验证入口：新运行显式绑定 parentBenchmarkRunId/repairId/changeSetId/appliedRevision，验证读取父子两份持久化 RuntimeArtifact，并要求技术 completed、自动断言和最新人工判定均形成明确结论；任一失败即记为 failed，待定或阻断不能验证。HTTP、存储测试与可见 Chrome 已完成 runtime 流程验收；尚缺真实 Docker 条件下 post-repair completed 的页面实操、文档/节点/条件等其他修复类型、失败状态页面实操和多轮修复关系图，因此保持进行中。
...
- **当前进度**：已完成 macOS arm64 Chrome 150 的可见页面规模专项。100 篇关联文档记录列表加载、首/中/末搜索和正文打开；500 节点/499 边连通图记录加载、首/中/末定位、类型筛选、关系跳转、缩放和平移；两项均检查 390x844 移动布局，未出现控制台错误、失败响应或横向溢出。尚缺 Windows 对照、Workspace 多 Skill 压力、路径隔离/脱敏汇总和异常恢复矩阵，因此保持进行中。
